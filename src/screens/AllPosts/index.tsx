...
export const AllPosts = () => {
  ...
  const getAllPosts = async () => {
    try {
      const response = await API.get(`/api/v1/publish?page=0&size=${size}`);
  ...
  const getPage = async (currentPage: number) => {
    try {
      const response = await API.get(
        `/api/v1/publish?page=${currentPage}&size=${size}`
      );
        ...
        <>
          {posts.length > 0 ? (
            <>
              <div className="grid grid-cols-3 gap-x-7 gap-y-14">
                {posts.map((element) => {
                  return <Post key={posts.indexOf(element)} post={element} />;
                })}
              </div>
              <ReactPaginate
